...
async function scoreBatch(
  ...
  tokenizerInstance: NonNullable<typeof tokenizer>,
  modelInstance: NonNullable<typeof model>,
): Promise<number[]> {
  const passages = results.map((result) => result.chunk.text);
  ...
  const scores = (await logits.sigmoid().tolist()) as number[][];

  return scores.map((score) => Number(score?.[0] ?? 0));
}

export const rerankerService = {
  async rerank<T extends SearchResult>(
    question: string,
    results: T[],
  ): Promise<T[]> {
    if (results.length === 0) {
    ...
    try {
      ...
      if (!tokenizerInstance || !modelInstance) {
        throw new Error("Reranker model failed to initialize");
      }

      const scoredResults: Array<{ result: T; score: number }> = [];
      ...
      throw {
        code: "RERANKER_ERROR",
        message: `Failed to rerank results: ${error?.message || error}`,
        statusCode: 500,
      };
    }
  },
